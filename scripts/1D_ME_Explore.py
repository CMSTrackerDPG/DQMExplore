...
def main(me_name, run_number, ref_run):
    ...
    data_current, ls, x_min, x_max, x_bin, y_min, y_max = utils.data_1d_me(
        run_number, me_name, regex
    )
    print("Loaded current run...")
    data_ref, ls_ref, x_min_ref, x_max_ref, x_bin_ref, y_min_ref, y_max_ref = (
        utils.data_1d_me(ref_run, me_name, regex)
    )
    ...
    utils.plot_1d_me_normTrig_ref_overlay(
        data_current,
        data_ref,
        trigger_rate,
        trigger_rate_ref,
    ...
    print("Plotted 1D ME norm Trig with ref overlaid......")
    utils.plot_1d_me_normTrig_ref_overlay_approval(
        data_current,
        data_ref,
        trigger_rate,
        trigger_rate_ref,
    ...
    list_means, list_std, list_good_ls, empty_ls = utils.calc_trends(
        data_current, ls, max_value_charge, trigger_rate
    )
    list_means_norm, list_std_norm, list_good_ls, empty_ls = utils.calc_trends(
        data_current, ls, max_value_charge, trigger_rate, norm=True
    )
    ...
    max_value_charge = x_max
    list_means_ref, list_std_ref, list_good_ls_ref, empty_ls_ref = utils.calc_trends(
        data_ref, ls_ref, max_value_charge, trigger_rate_ref
    )
    list_means_norm_ref, list_std_norm_ref, list_good_ls_ref, empty_ls_ref = (
        utils.calc_trends(
            data_ref, ls_ref, max_value_charge, trigger_rate_ref, norm=True
        )
    )
    ...
    inf_indices = [
        index for index, value in enumerate(list_means_norm_ref) if math.isinf(value)
    ]
